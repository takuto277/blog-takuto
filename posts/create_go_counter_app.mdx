...
![カウンターアプリのスクリーンショット](/images/counter-app-screenshot.png)
...
初期表示時：
ブラウザがロードされると、TypeScriptのfetchCounter関数が実行されます
この関数はGET /counterエンドポイントにリクエストを送信
バックエンドはSQLiteからカウンター値を取得して返します
返された値がブラウザに表示されます
2. 増加ボタンクリック時：
incrementCounter関数が呼び出されます
PUT /counter/incrementエンドポイントにリクエストを送信
バックエンドでSQLのUPDATEクエリが実行され、カウンター値が1増加
新しい値がレスポンスとして返され、ブラウザに表示されます
リセットボタンクリック時：
resetCounter関数が呼び出されます
PUT /counter/resetエンドポイントにリクエストを送信
